...
class Gost: #34.12-2015
	def __init__(self, cryptoKey): # in: str
		self.table = bytearray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
								...
								0xbe, 0x41, 0x83, 0x7c, 0xc4, 0x3b, 0xf0, 0x06])
		...
		self.keys = self.keyGer(self.share(self.strToInt(cryptoKey), 32))
	...
	def passGer(self, length, sym=True): # in: int/bool, out: str
		try:
			length = int(length)
		except:
		...
		if (sym):
			for i in length:
				if (i % 4 == 0):
					out += chr(random.randint(48, 57))
				elif (i % 4 == 1):
					out += chr(random.randint(65, 90))
				elif (i % 4 == 2):
					out += chr(random.randint(97, 122))
				else:
					out += symbols[random.randint(0, len(symbols) - 1)]
		else:
			for i in length:
				if (i % 3 == 0):
					out += chr(random.randint(48, 57))
...
if __name__ == '__main__':
	...
	crypt = Gost('Denis945518218456!')
	print(crypt.enc(pass1))
	print(crypt.dec(pass2))
	
	print(crypt.jsStrToInt(pass2))

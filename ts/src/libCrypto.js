...
class Gost {
    ...
    constructor(key) {
        ...
        this.keys = this.keyGer((0, encoding_1.hexStringToIntArray)(key));
    }
    keyGer(key) {
        let keys = [key.slice(0, 16), key.slice(16)], temp = [], left = keys[0], right = keys[1];
        for (let i = 0; i < 4; i++) {
            for (let k = 0; k < 8; k++) {
                temp = left;
    ...
    jsIntToStr = (indata) => indata
        .map((item, i, arr) => i != arr.length - 1 ? `${item.toString()};` : item.toString())
        .join("");
    share(indata, len) {
        if (len == 32) {
    ...
    enc(indata) {
        let data = (0, encoding_1.encode)(indata), crypted = [], temp = [];
        while (data.length != 0) {
            if (data.length < 16) {
    ...
    dec(indata) {
        let data = this.jsStrToInt(indata), decrypted = [];
        while (data.length != 0) {
            decrypted = decrypted.concat(this.decrypt(data.slice(0, 16)));
            data = data.slice(16);
        }
        decrypted = decrypted.filter((item) => item != 0);
        return (0, encoding_1.decode)(decrypted);
    }
}
exports.Gost = Gost;
const key = "8899aabbccddeeff0011223344556677fedcba98765432100123456789abcdef", ctkey = "bb44e25378c73123a5f32f73cdb6e51772e9dd7416bcf45b755dbaa88e4a4043", pt = "1122334455667700ffeeddccbbaa9988", ct = "7f679d90bebc24305a468d42b9d4edcd";
let msg = {
    pt: "что-то",
    key: "ключ",
...
console.log((0, encoding_1.intArrayToHexString)(crypt.encrypt((0, encoding_1.hexStringToIntArray)(pt))));
console.log((0, encoding_1.intArrayToHexString)(crypt.decrypt((0, encoding_1.hexStringToIntArray)(ct))));

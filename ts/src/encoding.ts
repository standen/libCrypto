...
export const passwordSymbols: string[][] = [
  vocab.numbers,
  vocab.landEngSmall,
  vocab.landEngBig,
  vocab.symbols,
...
export const encode = (indata: string): number[] =>
  Array.from(indata).map((item) => encoding.indexOf(item));

export const decode = (indata: number[]): string =>
  indata.map((item) => encoding[item]).join("");
